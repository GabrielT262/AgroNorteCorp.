...
export function RequestsClient({ initialOrders, currentUser }: RequestsClientProps) {
  ...
  const userPending = React.useMemo(() => initialOrders.filter(o => o.requestingArea === currentUser.area && o.status === 'Pendiente'), [initialOrders, currentUser.area]);
  const userHistory = React.useMemo(() => initialOrders.filter(o => o.requestingArea === currentUser.area && o.status !== 'Pendiente'), [initialOrders, currentUser.area]);
  ...
  const handleReject = (order: RecentOrder) => handleAction(() => rejectOrderAction(order.id, order.requestingArea), `Pedido ${order.id} Rechazado`, "No se pudo rechazar el pedido.");
        ...
        <TableBody>
          {orders.map(order => (
            <TableRow key={order.id}>
              ...
              <TableCell>{order.requestingUserName} ({order.requestingArea})</TableCell>
              <TableCell>
