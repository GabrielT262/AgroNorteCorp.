...
import type { UserRole, ExpiringProduct, RecentOrder, ManagedUser } from '@/lib/types';
import Link from 'next/link';
import { getDashboardData, getCurrentUser } from '@/lib/db';
import { ApproveOrderButton } from './approve-order-button';
import DashboardProvider from './dashboard-provider';

export const dynamic = 'force-dynamic';

const AdminDashboardTables = ({ recentOrders, expiringProducts, currentUser }: { recentOrders: RecentOrder[], expiringProducts: ExpiringProduct[], currentUser: ManagedUser }) => {
  ...
  const userCanApprove = canApproveRoles.includes(currentUser.role);
  const createLink = (path: string) => `${path}?userId=${currentUser.id}`;
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Link href={createLink("/dashboard/requests")}>
                ...
                {recentOrders.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell className="text-right">
                      {order.status === 'Pendiente' && userCanApprove ? (
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Link href={createLink("/dashboard/inventory")}>
            <Button variant="ghost" size="sm">
...
const OperarioDashboard = ({ currentUser }: { currentUser: ManagedUser }) => {
  const createLink = (path: string) => `${path}?userId=${currentUser.id}`;
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="flex flex-col gap-4 sm:flex-row">
        <Link href={createLink("/dashboard/inventory")} className="w-full">
          <Button className="w-full" size="lg">
            <Package className="mr-2" />
            Ver Inventario
          </Button>
        </Link>
         <Link href={createLink("/dashboard/requests")} className="w-full">
...
export default async function DashboardPage({ searchParams }: { searchParams: { userId?: string } }) {
  ...
  const adminRoles: UserRole[] = ['Administrador', 'Gerencia'];
  ...
  return (
    <DashboardProvider searchParams={searchParams}>
        ...
        <div>
            <h1 className="text-3xl font-bold tracking-tight">Bienvenido, {currentUser.name} {currentUser.last_name}</h1>
            <p className="text-muted-foreground">
            {isAdmin
                ? 'Este es un resumen general de la operación.'
                : 'Un resumen de los recursos disponibles.'}
            </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Link href={createLink("/dashboard/fuel")}>
            <Card className="hover:bg-muted/80 transition-colors h-full">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                    Nivel de Gasolina
        ...
        {isAdmin ? (
            <AdminDashboardTables recentOrders={recentOrders} expiringProducts={expiringProducts} currentUser={currentUser} />
        ) : (
            <OperarioDashboard currentUser={currentUser} />
        )}
        </div>
    </DashboardProvider>
  );
